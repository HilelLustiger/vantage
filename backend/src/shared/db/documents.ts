...
export async function findActiveDuplicate(accountId: string, checksum: string, excludeId: string) {
  const [document] = await db
    .select()
    .from(documents)
    .where(
...
export async function listDocumentsForUser(userId: string) {
  ...
  const rows = await db.select().from(documents).where(inArray(documents.accountId, accountIds));
