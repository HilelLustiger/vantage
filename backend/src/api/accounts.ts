...
import { z } from "zod";
import type { Account } from "../dto/index.js";
import {
...
accountsRouter.post("/", async (req, res) => {
  ...
  const ownerUserIds = [...new Set([req.session.userId!, ...(parsed.data.ownerUserIds ?? [])])];
...
accountsRouter.get("/", async (req, res) => {
  res.status(200).json((await listAccountsForUser(req.session.userId!)) satisfies Account[]);
});
