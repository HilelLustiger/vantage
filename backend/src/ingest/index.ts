...
export const ingest: IngestModule = {
  async startImport(documentId: string): Promise<void> {
    ...
    if (document.status !== "uploaded") {
      throw new Error(`Document ${documentId} is "${document.status}", expected "uploaded"`);
    }

    const duplicate = await findActiveDuplicate(document.accountId, document.checksum, document.id);
    if (duplicate) {
