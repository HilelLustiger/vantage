...
describe("getExchangeRate", () => {
  ...
  it("fetches and caches on a cache miss", async () => {
    ...
    const fetchMock = vi.fn(
      async () =>
        new Response(JSON.stringify({ amount: 1, base: "USD", date, rates: { ILS: 3.5 } })),
    );
    vi.stubGlobal("fetch", fetchMock);
  ...
  it("skips the network call on a cache hit", async () => {
    ...
    const fetchMock = vi.fn(
      async () =>
        new Response(JSON.stringify({ amount: 1, base: "USD", date, rates: { ILS: 3.7 } })),
    );
    vi.stubGlobal("fetch", fetchMock);
  ...
  it("throws on a non-2xx response", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async () => new Response("bad currency pair", { status: 404 })),
    );
