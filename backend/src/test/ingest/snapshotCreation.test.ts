...
import { commitSnapshot, parseStatementDate } from "../../ingest/snapshotCreation.js";
...
describe("commitSnapshot", () => {
  ...
  it("does not derive a cost_basis_delta when the document also has real transactions (double-count guard)", async () => {
    ...
    expect(flows[0]).toEqual(
      expect.objectContaining({ kind: "buy", source: "ingested_transaction" }),
    );
  });
